...
class TableProcessor {
	...
	beginTable(writer) {
		// vertical alignment: track row heights
		...
		// vertical alignment: store table start Y
	...
	beginRow(rowIndex, writer) {
		...
		// vertical alignment: track row top position
		if (this.tableNode.table.rowsHeight) {
			this.tableNode.table.rowsHeight[rowIndex] = { top: this.rowTopY, height: undefined };
		}
	}
	...
	// vertical alignment helper methods
		...
		table.body.forEach((row, rowIndex) => {
			...
			cells.forEach(cell => {
				if (!cell._span && cell.verticalAlign && ['middle', 'bottom'].indexOf(cell.verticalAlign) > -1) {
					// manage vertical alignment (middle or bottom) for all rows in the table
	...
	endTable(writer) {
		// vertical alignment: compute and apply
